...
const db = (() => {
     ...
     */
    const serverEqualize = ({ contents: con, comments: com }: { contents: Content[], comments: Comment[] }) => {
        contents = con;
        comments = com;
    }
    ...
    const postContent = delaiedFn(({ article, title }: { article: string, title: string }) => {
        const sanitized = sanitizeHtml(article, { disallowedTagsMode: "escape" });
        const newContent = CreateContentMock(randomUUID() as string, title, sanitized);
    ...
    const putContent = delaiedFn((content: Content) => {
        ...
        const sanitized = sanitizeHtml(content.article, { disallowedTagsMode: "escape" });
        contents = replace({ ...content, article: sanitized }, targetIndex, contents)
        return content;
    })
