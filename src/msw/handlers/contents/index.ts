...
const clientEqualize: Parameters<typeof rest.post>[1] = async (
  req,
  res,
  ctx
) => {
  const data = await req
    .json<{ contents: Content[]; comments: Comment[] }>()
...
const getContent: Parameters<typeof rest.get>[1] = async (req, res, ctx) => {
  ...
  if (!contentId) return res(ctx.status(400));
...
const postContent: Parameters<typeof rest.post>[1] = async (req, res, ctx) => {
  ...
  if (content === null) return res(ctx.status(400));

  const data = await DB.postContent(content).catch((e) =>
    Promise.resolve(null)
  );

  if (content === null) return res(ctx.status(500));
...
const putContent: Parameters<typeof rest.put>[1] = async (req, res, ctx) => {
  ...
  if (data === null) return res(ctx.status(500));
...
const deleteContent: Parameters<typeof rest.delete>[1] = async (
  req,
  res,
  ctx
) => {
  const { contentId } = req.params;
  if (typeof contentId !== 'string') return res(ctx.status(400));
  const data = await DB.deleteContent(contentId).catch((e) =>
    Promise.reject(null)
  );
  if (data === null) return res(ctx.status(500));
...
const postComment: Parameters<typeof rest.post>[1] = async (req, res, ctx) => {
  ...
  const { contentId } = req.params;
  if (parsedBody === null || typeof contentId !== 'string')
    return res(ctx.status(400));
  const { article } = parsedBody;

  const data = await DB.postComment({ article, articleId: contentId }).catch(
...
const putConmment: Parameters<typeof rest.put>[1] = async (req, res, ctx) => {
  ...
  if (typeof articleId !== 'string' || typeof id !== 'string')
    return res(ctx.status(400));

  const { article } = await req
    .json<{ article: string }>()
    .catch((e) => Promise.resolve({ article: null }));
  if (article === null) return res(ctx.status(400));
  const data = await DB.putComment({ id, articleId, article }).catch((e) =>
    Promise.resolve(null)
  );
  if (data === null) return res(ctx.status(500));
...
const deleteComment: Parameters<typeof rest.delete>[1] = async (
  req,
  res,
  ctx
) => {
  ...
  const data = await DB.deleteComment({ id, articleId }).catch((e) =>
    Promise.resolve(null)
  );
  if (data === null) return res(ctx.status(500));
